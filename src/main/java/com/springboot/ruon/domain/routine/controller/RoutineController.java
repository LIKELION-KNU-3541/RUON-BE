...
public class RoutineController {
    ...
    public ResponseEntity<ApiResponse<RoutineResponse>> reflectTodayCondition(
        ...
        RoutineResponse response = routineService.reflectTodayCondition(userDetails.getUserId(), request);
    ...
    public ResponseEntity<ApiResponse<RoutineResponse>> getTodayRoutine(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        RoutineResponse response = routineService.getTodayRoutine(userDetails.getUserId());
    ...
    public ResponseEntity<ApiResponse<StepResponse>> updateStepStatus(
        ...
        StepResponse response = routineService.updateStepStatus(
                userDetails.getUserId(), routineId, stepId, request.status());
        return ResponseEntity.ok(ApiResponse.success(response));
    }
    ...
    public ResponseEntity<ApiResponse<RoutineResponse>> recordReaction(
        ...
        RoutineResponse response = routineService.recordReaction(
                userDetails.getUserId(), routineId, request.score());
        return ResponseEntity.ok(ApiResponse.success(response));
    }

    // 내일 루틴 추천 - 저장하지 않고 호출할 때마다 즉석 계산. 반응 기록이 있는 가장 최근 루틴 기준.
    @GetMapping("/tomorrow")
    public ResponseEntity<ApiResponse<TomorrowRoutineResponse>> getTomorrowRoutine(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        TomorrowRoutineResponse response = routineService.getTomorrowRoutine(userDetails.getUserId());
        return ResponseEntity.ok(ApiResponse.success(response));
    }
}
